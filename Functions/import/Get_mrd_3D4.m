...
fseek(fid,256,'bof');
text=fread(fid,256);
no_samples = xdim;  %disp(['xdim = ',num2str(xdim)]);
no_views = ydim;    %disp(['ydim = ',num2str(ydim)]);
no_views_2 = zdim;  %disp(['zdim = ',num2str(zdim)]);
no_slices = dim4;
no_echoes = dim5;
no_expts = dim6;
...
for a=1:no_expts
    for b=1:no_echoes
        for c=1:no_slices
            for d=1:no_views
                for e=1:no_views_2
%                     m_C_1(:,:,d,c,b,a) = m_C(:,:,d,c,b,a)';
%                     m_C_1(a,b,c,d,no_views+1-e,:) = m_C(1+n:no_samples+n); % sequential ordering
...
if numel(ppr_text)>0
    ...
    %             C = textscan(char1, '%*s %s') % filename or OBSERVE_FREQUENCY
                C = textscan(char1, '%*s %[^\n]');
